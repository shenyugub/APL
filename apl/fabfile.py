...
_supervisor_config_path = '/etc/supervisor/conf.d'


def backup_configs():
    with cd(_nginx_config_path):
        get('default', './server_config_backup')
    with cd(_supervisor_config_path):
        get('apl.conf', './server_config_backup')


def deploy_configs():
    put('./server_config_backup/default', _nginx_config_path)
    put('./server_config_backup/apl.conf', _supervisor_config_path)
...
def build():
    ...
    excludes = ['backup', 'docs', 'release', 'venv', '__pycache__', 'fabfile.py', 'README.md', 'development.py', 'options.txt', 'server_config_backup']
